---
title: "Bayesian A/B Analysis Report"
format: html
execute: 
  echo: false
jupyter: python3
...
Moving the intitial gate from level 30 to level 40 does not appear to improve player retention. In fact, it shows a slight negative impact on 
both 1-day and 7-day retention rates.

```{python}
import pandas as pd
ret_1_summary = pd.read_csv("artifacts/ret1_uplift_40.csv")
ret_1_summary = ret_1_summary.transpose().map(
    lambda x: f"{x * 100:.2f}%"
)
uplift_1_mean = ret_1_summary["mean"].values[0]
...
The relative uplift for gate 40 is `{python} uplift_1_mean` indicating a slight decrease in retention rate 